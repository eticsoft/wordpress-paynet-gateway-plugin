...
/*
  ...
  Plugin URI:  https://eticsoft.com/shop/tr/odeme-modulleri/58-ucretsiz-woocommerce-paynet-entegrasyon-modulu.html
...
if (!defined('_Paynet_API_URL_'))
	define('_Paynet_API_URL_', 'https://service.paynet.com/PaymentDealer/DoDirectPayment');
if (!defined('_Paynet_3D_URL_'))
	define('_Paynet_3D_URL_', 'https://service.paynet.com/PaymentDealer/DoDirectPaymentThreeD');

/* Paynet All Load */
add_action('plugins_loaded', 'init_eticsoft_paynet_gateway_class', 0);

function init_eticsoft_paynet_gateway_class()
{
	if (!class_exists('WC_Payment_Gateway'))
		return;

	class eticsoft_paynet extends WC_Payment_Gateway
	{
		/*
		...
		function __construct()
		{
			...
			$this->init_settings();
			$this->terms = get_option('eticsoft_paynet_terms');
			$this->version = 1.0;
			$this->id_eticsoft = 32;
			$this->key_eticsoft = 'c8be48db180b7824ddc63fa7bde50244';
			$this->curversionurl = 'http://api.eticsoft.net/license/';

			foreach ($this->settings as $setting_key => $value)
				$this->$setting_key = $value;
			//Register the style
			add_action('admin_enqueue_scripts', array($this, 'register_eticsoft_paynet_admin_styles'));
			...
			//Paynet SETTINGS
			$paynet_settings = get_option("woocommerce_eticsoft_paynet_settings");
			foreach ($paynet_settings as $k => $v)
				$this->{$k} = $v;
		}

// End __construct()
		public function register_eticsoft_paynet_admin_styles()
		{
			wp_enqueue_style('admin-styles', plugins_url() . '/woocommerce/assets/css/admin.css');
			wp_register_style('eticsoft_paynet-admin', plugins_url('css/eticsoft_paynet-admin.css', __FILE__));
			wp_enqueue_style('eticsoft_paynet-admin');
		}

		public function admin_options()
		{
			if (isset($_POST['confirm_eticsoft_paynet_register']) AND $_POST['confirm_eticsoft_paynet_register']) {
				update_option('eticsoft_paynet_terms', true);
				$this->registerPaynet();
			}
			if (!get_option('eticsoft_paynet_terms')) {
			...
			echo '<table class="form-table"><img src="' . $eticsoft_paynet_url . 'img/paynet_logo.png" width="150px"/>';
			...
			include(dirname(__FILE__) . '/includes/eticsoft_paynet-help-about.php');
		}
		/* 	Admin Panel Fields */
		...
		public function init_form_fields()
		{
			$this->form_fields = array(
				...
				'logo_url' => array(
					'title' => __('URL to your Logo', 'woocommerce'),
					'type' => 'text',
				...
				'add_commission' => array(
					...
					'default' => 'on',
					'options' => array(
						'on' => 'Komisyon oranını müşteriye yansıt',
						'off' => 'Komisyon oranını mağaza karşılayacak',
					),
				...
				'allow_installments' => array(
					...
					'default' => 'on',
					'options' => array(
						'on' => 'Açık (Ödeme formuna taksit seçenekleri eklenir)',
						'off' => 'Kapalı (Tüm ödemeler tek çekim yapılır)',
					)
				),
				'force_tds' => array(
					'title' => '3DS zorunluluğu',
					'type' => 'select',
					'default' => false,
					'desc_tip' => '3DS ile alışverişte müşterinizin cep telefonuna SMS doğrulama kodu gider',
					'options' => array(
						true => 'Tüm ödemeler 3DS ile alınsın',
						false => 'Kapalı',
					)
				),
				'test_mode' => array(
					...
					'options' => array(
						'prod' => 'Gerçek ortam',
						'test' => 'Test Modu (Test amaçlıdır, gerçek tahsilat yapılmaz)',
					),
				),
			);
		}
...
//END process_payment
		...
		{
			return isset($_POST['eticsoft_paynet-card-number']) && isset($_POST['eticsoft_paynet-card-name']) && isset($_POST['eticsoft_paynet-card-expiry']) && isset($_POST['eticsoft_paynet-card-cvc']) && isset($_POST['eticsoft_paynet_selected_installment']);
		}
...
//END validate_fields
		...
		public function createCheckoutEmbedForm($id_order)
		{
			...
			$txt = '
	            ...
	            <script type="text/javascript"
	                    ...
	                    data-amount= '.(int)($order->get_total()*100).'
	                    data-image="'.$this->logo_url.'"
	                    data-button_label="'.__('Pay with Credit Card', 'woocommerce').'"
	                    data-description="Ödemenizi tamamlamak için bilgileri girip tamam butonuna basınız"
	                    data-agent="'.$this->paynet_dataagent.'"
	                    data-add_commission_amount="'.($this->add_commission ? 'true': 'false').'"
	                    data-no_instalment="'.($this->allow_installments ? 'false': 'true').'"
		...
		{
			if($use_cache){
				if(	$cache = get_option('eticsoft_paynet_rates_cache'))
					return $cache;
			}
			try {
				$paynet = new PaynetClient($this->secretkey, $this->test_mode);
			...
			update_option('eticsoft_paynet_rates_cache', $rates);
			return $rates;
		}
		/*
		...
		public function post2Paynet($order_id)
		{
			...
			$ins = (int) $_POST['eticsoft_paynet_selected_installment']; // BUNA BAK*********
			...
			$expire_date = explode('/', $_POST['eticsoft_paynet-card-expiry']);
			


			$gateway_url = $this->eticsoft_paynet_tdmode == 'off' ? _Paynet_API_URL_ : _Paynet_3D_URL_;
			...
			if (isset($result->ResultCode) AND $result->ResultCode == "Success") {
				if ($this->eticsoft_paynet_tdmode != 'off')
				...
				try{
					...
					if($result->is_succeed == true){
						if ($result->comission > 0) {
							...
							$order_fee->name = 'Kredi kartı komisyon farkı '. $result->instalment. ' taksit '. $result->comission;
						...
						$order->update_status('pending', 'Pending payment', 'eticsoft-payment');
	...
	function eticsoft_paynet($methods)
	{
		$methods[] = 'eticsoft_paynet';
		return $methods;
	}
	add_filter('woocommerce_payment_gateways', 'eticsoft_paynet');
	add_filter('woocommerce_product_tabs', 'eticsoft_paynet_installment_tab');
	add_action('woocommerce_order_actions_start', 'eticsoft_paynet_order_details');
	
	function paynet_enqueue_style_after_wc(){
        wp_enqueue_style( 'my-style', plugins_url().'/eticsoft_paynet/css/product_tab.css', array(), '2.0' );
	}
	add_action( 'wp_enqueue_scripts', 'paynet_enqueue_style_after_wc', 20 );

	function eticsoft_paynet_order_details($id)
	{
		$order = New WC_Order($id);
		if ($order->get_payment_method() != 'eticsoft_paynet')
			return;
		...
		if(!$xid OR $xid == 1 OR is_array($xid))
			return;
		$eticsoft_paynet = New eticsoft_paynet();
		
		$transaction = $eticsoft_paynet->getTransactionDetails($xid);
		...
		<div class="align-center" align="center">
			<br/><img src="<?php echo plugins_url() ?>/eticsoft_paynet/img/paynet_logo.png" width="150px"/>
		...
		<table class="widefat fixed centered center">
			...
			<tr>
				<td>Sonuç </td><td> <?php echo $tr->message ?> <?php echo $tr->id_tds ? '3D ile ödendi' : '' ?></td>
			</tr>
			<tr>
	...
	function eticsoft_paynet_installment_tab($tabs)
	{
		...
		$tabs['test_tab'] = array(
			...
			'priority' => 50,
			'callback' => 'eticsoft_paynet_installment_tab_content'
		);

		return $tabs;
	}

	function eticsoft_paynet_installment_tab_content()
	{
		global $woocommerce;
		global $product;
		$rates = new eticsoft_paynet();
